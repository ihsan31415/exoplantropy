"""Unified MLP (Multilayer Perceptron) trainer for the TESS, Kepler, and K2 catalogues."""
...
def build_pipeline(random_state: int = 42) -> Pipeline:
    """Create the modelling pipeline for an MLP classifier."""
    return Pipeline(
        steps=[
...
def parse_args(args: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an MLP model for exoplanet catalogues.")
    parser.add_argument(
        "dataset",
        nargs="?",
        default="kepler",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of top features to export using permutation importance.",
    )
    parser.add_argument(
        "--permutation-repeats",
        type=int,
        default=15,
        help="Number of repeats for permutation importance (higher is more precise but slower).",
    )
...
def main(cli_args: Iterable[str] | None = None) -> None:
    ...
    model_suffix = f"{dataset_key}_mlp"
    ...
    top_features = collect_feature_importance(
        pipeline,
        X_test,
        y_test,
        X_train.columns,
    ...
    print(f"{dataset_name} MLP training complete. Key metrics:")
