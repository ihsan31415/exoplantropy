"""Train and evaluate a CatBoost model on the K2 planet candidate catalogue."""
...
def main() -> None:
    ...
    # Persist model
    model_path = MODELS_DIR / "k2_catboost.joblib"
    dump(pipeline, model_path)

    # Save metrics
    metrics_output = REPORTS_DIR / "k2_catboost_metrics.json"
    with metrics_output.open("w", encoding="utf-8") as fp:
        json.dump(metrics, fp, indent=2)

    # Plot confusion matrix
    y_pred = pipeline.predict(X_test)
    cm_path = REPORTS_DIR / "k2_catboost_confusion_matrix.png"
    save_confusion_matrix(y_test, y_pred, cm_path)

    # Feature importances
    top_features = collect_feature_importance(pipeline, X_train.columns)
    fi_path = REPORTS_DIR / "k2_catboost_top_features.csv"
    top_features.to_csv(fi_path, index=False)

    print("K2 CatBoost training complete. Key metrics:")
