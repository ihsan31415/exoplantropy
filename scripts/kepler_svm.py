"""Train and evaluate an SVM model on the Kepler KOI cumulative catalogue."""
...
def main() -> None:
    ...
    # Persist model
    dump(pipeline, MODELS_DIR / "kepler_svm.joblib")

    # Save metrics
    metrics_output = REPORTS_DIR / "kepler_svm_metrics.json"
    with metrics_output.open("w", encoding="utf-8") as fp:
        json.dump(metrics, fp, indent=2)

    # Plot confusion matrix
    y_pred = pipeline.predict(X_test)
    save_confusion_matrix(y_test, y_pred, REPORTS_DIR / "kepler_svm_confusion_matrix.png")

    print("Kepler SVM training complete. Key metrics:")
    for metric in ("accuracy", "precision", "recall", "f1", "roc_auc"):
        print(f"  {metric}: {metrics[metric]:.4f}")

    print(f"Detailed metrics saved to {metrics_output}")
    print(f"Model saved to {MODELS_DIR / 'kepler_svm.joblib'}")
