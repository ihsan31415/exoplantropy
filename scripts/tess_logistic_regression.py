"""Train and evaluate a Logistic Regression model on the TESS TOI catalogue."""
...
def main() -> None:
    ...
    # Persist model
    dump(pipeline, MODELS_DIR / "tess_logistic_regression.joblib")

    # Save metrics
    metrics_output = REPORTS_DIR / "tess_logistic_regression_metrics.json"
    ...
    save_confusion_matrix(
        y_test,
        y_pred,
        REPORTS_DIR / "tess_logistic_regression_confusion_matrix.png",
    )
    ...
    save_top_coefficients(
        model,
        X_train.columns,
        REPORTS_DIR / "tess_logistic_regression_top_coefficients.csv",
    )

    print("Logistic Regression training complete. Key metrics:")
    for metric in ("accuracy", "precision", "recall", "f1", "roc_auc"):
        print(f"  {metric}: {metrics[metric]:.4f}")

    print(f"Detailed metrics saved to {metrics_output}")
    print(f"Model saved to {MODELS_DIR / 'tess_logistic_regression.joblib'}")
