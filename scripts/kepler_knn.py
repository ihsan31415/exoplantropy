"""Train and evaluate a K-Nearest Neighbors model on the Kepler KOI cumulative catalogue.
...
Outputs:
- reports/kepler_knn_metrics.json: summary metrics and cross-validation scores
- reports/kepler_knn_confusion_matrix.png: confusion matrix heatmap
- models/kepler_knn.joblib: trained pipeline for reuse

Usage:
    python scripts/kepler_knn.py
"""
...
def main() -> None:
    ...
    # Persist model
    dump(pipeline, MODELS_DIR / "kepler_knn.joblib")

    # Save metrics
    metrics_output = REPORTS_DIR / "kepler_knn_metrics.json"
    with metrics_output.open("w", encoding="utf-8") as fp:
        json.dump(metrics, fp, indent=2)

    # Plot confusion matrix
    y_pred = pipeline.predict(X_test)
    save_confusion_matrix(y_test, y_pred, REPORTS_DIR / "kepler_knn_confusion_matrix.png")

    print("Kepler KNN training complete. Key metrics:")
    for metric in ("accuracy", "precision", "recall", "f1", "roc_auc"):
        print(f"  {metric}: {metrics[metric]:.4f}")

    print(f"Detailed metrics saved to {metrics_output}")
    print(f"Model saved to {MODELS_DIR / 'kepler_knn.joblib'}")
