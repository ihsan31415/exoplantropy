"""Unified Gradient Boosting (GBM) trainer for the TESS, Kepler, and K2 catalogues."""
...
def build_pipeline(random_state: int = 42) -> Pipeline:
    """Create the modelling pipeline for Gradient Boosting."""
    return Pipeline(
        steps=[
...
def parse_args(args: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Gradient Boosting model for exoplanet catalogues.")
    parser.add_argument(
        "dataset",
        nargs="?",
        default="kepler",
...
def main(cli_args: Iterable[str] | None = None) -> None:
    ...
    model_suffix = f"{dataset_key}_gbm"
    ...
    print(f"{dataset_name} Gradient Boosting training complete. Key metrics:")
