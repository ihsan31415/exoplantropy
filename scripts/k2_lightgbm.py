"""Train and evaluate a LightGBM model on the K2 planet candidate catalogue."""
...
def main() -> None:
    ...
    # Persist model
    dump(pipeline, MODELS_DIR / "k2_lightgbm.joblib")

    # Save metrics
    metrics_output = REPORTS_DIR / "k2_lightgbm_metrics.json"
    with metrics_output.open("w", encoding="utf-8") as fp:
        json.dump(metrics, fp, indent=2)

    # Plot confusion matrix
    y_pred = pipeline.predict(X_test)
    save_confusion_matrix(y_test, y_pred, REPORTS_DIR / "k2_lightgbm_confusion_matrix.png")

    # Feature importances
    top_features = collect_feature_importance(pipeline, X_train.columns)
    top_features.to_csv(REPORTS_DIR / "k2_lightgbm_top_features.csv", index=False)

    print("K2 LightGBM training complete. Key metrics:")
    for metric in ("accuracy", "precision", "recall", "f1", "roc_auc"):
        print(f"  {metric}: {metrics[metric]:.4f}")

    print(f"Detailed metrics saved to {metrics_output}")
    print(f"Model saved to {MODELS_DIR / 'k2_lightgbm.joblib'}")
